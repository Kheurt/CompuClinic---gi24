...
class Poste(models.Model):
    ...
    # service = models.ForeignKey('plateau_technique.Service', on_delete=models.CASCADE, null=True)
    # unite = models.ForeignKey('plateau_technique.UniteMedicale', on_delete=models.CASCADE, null=True)
...
class Stagiaire(Personnel):
    niveau = models.CharField(choices=SCHOOL_LEVEL, max_length=10, default='BAC+5')
    
    def save(self, *args, **kwargs):
        self.type_personnel = 'Stagiare'
        super().save(*args, **kwargs)
...
class Stage(models.Model):
    ...
    def __str__(self):
        return "Stage: " + self.theme


class RapportStage(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    rapport = models.FileField(upload_to='rapports_stage/', null=False, )
